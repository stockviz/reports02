...
etfs<-read.csv("../COUNTRY_ETFS.csv")
...
facetPlot <- ggplot(toPlot, aes(y=Y, x=variable, fill=color)) +
	...
	geom_tile() +
	geom_text(aes(label= ifelse(is.finite(value), sprintf("%+.2f%%", value), "")), hjust = "center", size=2.5) +
	scale_fill_identity() +
	guides(fill="none") +
	labs(x='', y='', title="Country ETF Returns ($)") +
